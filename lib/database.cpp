...
bool Database::createVocabulary(QString vocabulary, QString description) {
  m_query.prepare(
      "INSERT INTO vocabularies (vocabulary_name, vocabulary_description) "
...
bool Database::createWord(QString origin, QString translated,
                          int vocabulary_id) {
...
int Database::countWords(int vocabulary_id) {
  QString query =
      "SELECT COUNT(*) FROM words WHERE vocabulary = :vocabulary_id";
...
QVariantList Database::listWords(int vocabulary, int limit, bool sort,
  ...
  QString query = "SELECT id, origin, translated, progress FROM words WHERE "
...
QVariantList Database::listVocabularies() {
  ...
  m_query.exec(
      "SELECT id, vocabulary_name, vocabulary_description FROM vocabularies");
  while (m_query.next()) {
    int id = m_query.value(0).toInt();
